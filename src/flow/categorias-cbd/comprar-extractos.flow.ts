...
import extractos from "../../data/extractos-cbd.json";
import { FormattedProduct } from "src/types";
import { ChatCompletionMessageParam } from "openai/resources";
import { runVenderAceite, runVenderCosmetico, runVenderExtracto } from "src/services/openai";
import welcomeFlow from "../welcome.flow";
import comprarCbdFlow from "../comprar-cbd.flow";
import expertoFlow from "../experto.flow";

const extractosCBD: FormattedProduct[] = extractos.map((dato) => ({
  nombre: dato.Name,
...
.addAnswer(
  "¿Tienes algo de información acerca del extracto CBD que buscas o deseas que te recomendemos algún extracto CBD? "
)
.addAction(
  { capture: true },
  async (ctx, { state, gotoFlow, flowDynamic }) => {
    try {
      ...
      const largeResponse = await runVenderExtracto(
        name,
        newHistory,
        extractosCBD,
        ctx.body
  ...
  .addAnswer(" Encuentra estos y más extractos CBD en: https://ganeshgrowshop.com/es/extractos-cbd/ ")
  .addAnswer(
