...
import aceites from "../../data/aceites-cbd.json";
import { FormattedProduct } from "src/types";
import { ChatCompletionMessageParam } from "openai/resources";
import { runVender, runVenderAceite } from "src/services/openai";
import welcomeFlow from "../welcome.flow";
import comprarCbdFlow from "../comprar-cbd.flow";
import expertoFlow from "../experto.flow";

const floresCBD: FormattedProduct[] = aceites.map((dato) => ({
  nombre: dato.Name,
 ...
 */
export default BotWhatsapp.addKeyword(["Aceites CBD"])
.addAnswer(
  "¿Tienes algo de información acerca del aceite CBD que buscas o vienes sin información? "
)
.addAction(
  { capture: true },
  async (ctx, { state, gotoFlow, flowDynamic }) => {
    try {
      ...
      const largeResponse = await runVenderAceite(
        name,
        newHistory,
        floresCBD,
        ctx.body
  ...
  .addAnswer(" Encuentra estos y más aceites CBD en: https://ganeshgrowshop.com/es/aceite-cbd/ ")
  .addAnswer(
