...
import certificadas from "../../data/semillas/cañamo.json";
import { FormattedProduct } from "src/types";
import { ChatCompletionMessageParam } from "openai/resources";
import { runVenderMascota, runVenderSemillaCertificada, runVenderSemillaFeminizada } from "src/services/openai";
...
const semillasCertificadas: FormattedProduct[] = certificadas.map((dato) => ({
  nombre: dato.Name,
...
.addAction(
  ...
  async (ctx, { state, gotoFlow, flowDynamic }) => {
    try {
      ...
      const largeResponse = await runVenderSemillaCertificada(
        name,
        newHistory,
        semillasCertificadas,
        ctx.body
      );
  ...
  .addAnswer(" Encuentra estas y más semillas certificadas: https://ganeshgrowshop.com/es/semillas-ca%C3%B1amo-certificadas/ ")
  .addAnswer(
