...
import granel from "../../data/semillas/granel.json";
import { FormattedProduct } from "src/types";
import { ChatCompletionMessageParam } from "openai/resources";
import { runVenderMascota, runVenderSemillaAutofloreciente, runVenderSemillaFeminizada, runVenderSemillaGranel } from "src/services/openai";
...
const semillasGranel: FormattedProduct[] = granel.map((dato) => ({
  nombre: dato.Name,
 ...
 */
export default BotWhatsapp.addKeyword(["Semillas Granel"])
.addAnswer(
...
.addAction(
  ...
  async (ctx, { state, gotoFlow, flowDynamic }) => {
    try {
      ...
      const largeResponse = await runVenderSemillaGranel(
        name,
        newHistory,
        semillasGranel,
        ctx.body
  ...
  .addAnswer(" Encuentra estas y más semillas a granel: https://ganeshgrowshop.com/es/semillas-a-granel/ ")
  .addAnswer(
