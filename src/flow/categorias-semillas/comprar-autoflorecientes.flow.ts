...
import autoflorecientes from "../../data/semillas/autoflorecientes.json";
import { FormattedProduct } from "src/types";
import { ChatCompletionMessageParam } from "openai/resources";
import { runVenderMascota, runVenderSemillaAutofloreciente, runVenderSemillaFeminizada } from "src/services/openai";
...
const semillasAutoflorecientes: FormattedProduct[] = autoflorecientes.map((dato) => ({
  nombre: dato.Name,
 ...
 */
export default BotWhatsapp.addKeyword(["Semillas Autoflorecientes"])
.addAnswer(
...
.addAction(
  ...
  async (ctx, { state, gotoFlow, flowDynamic }) => {
    try {
      ...
      const largeResponse = await runVenderSemillaAutofloreciente(
        name,
        newHistory,
        semillasAutoflorecientes,
        ctx.body
      );
  ...
  .addAnswer(" Encuentra estas y más semillas autoflorecientes: https://ganeshgrowshop.com/es/semillas-marihuana-autoflorecientes/ ")
  .addAnswer(
