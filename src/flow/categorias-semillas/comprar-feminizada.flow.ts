...
import feminizadas from "../../data/semillas/feminizadas.json";
import { FormattedProduct } from "src/types";
import { ChatCompletionMessageParam } from "openai/resources";
import { runVenderMascota, runVenderSemillaFeminizada } from "src/services/openai";
...
const semillasFeminizadas: FormattedProduct[] = feminizadas.map((dato) => ({
  nombre: dato.Name,
...
.addAction(
  ...
  async (ctx, { state, gotoFlow, flowDynamic }) => {
    try {
      ...
      const largeResponse = await runVenderSemillaFeminizada(
        name,
        newHistory,
        semillasFeminizadas,
        ctx.body
  ...
  .addAnswer(" Encuentra estas y más semillas feminizadas: https://ganeshgrowshop.com/es/semillas-marihuana-feminizadas/ ")
  .addAnswer(
